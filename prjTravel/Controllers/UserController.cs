...
namespace prjTravel.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public UserController(TravelDbContext dbContext, IWebHostEnvironment hostEnvironment)
        {
        ...
        //    else
        //    {
        //        TempData["Error"] = $"很抱歉! 權限不足。";
        //    }

        //    return RedirectToAction("MemberList");
        //}

        //使用者按下 按下_ManagerLayout的Nav編輯個人資料時啟用
        ...
        //使用者按下 按下_ManagerLayout的Nav我的文章時啟用
        ...
        {
            var folder = _dbContext.Folders.FirstOrDefault(m => m.FfolderId == Fid && m.FcreateUser == User.FindFirst(ClaimTypes.Sid)!.Value);
        ...
        {
            var fold = _dbContext.Folders.FirstOrDefault(m => m.Fpicture == Fpicture && m.FcreateUser == User.FindFirst(ClaimTypes.Sid)!.Value);

            if (fold == null)
            {
                TempData["Error"] = "資料編輯失敗，請確認資料正確性!";
                return RedirectToAction("GoMemberAllFold");
            }
        ...
        //    catch (Exception ex)
        //    {
        //        TempData["Error"] = $"會員刪除失敗, {ex.Message}";
        //    }
